...
- [ ] 01-01-PLAN.md — Skeleton scaffold: Go module + pinned deps + config loader + `Renderer`/`Channel`/`NotificationService` seam interfaces

**Wave 2** *(blocked on Wave 1 completion)*

- [ ] 01-02-PLAN.md — Renderer slice: goldmark→bluemonday(fragment)→inline-styled wrapper, word-only status tag, CRLF strip (SEC-03, EMAIL-02/03, D-01..D-07)
- [ ] 01-03-PLAN.md — Email Channel slice: go-mail Gmail :587 STARTTLS multipart send, fixed recipient, message-id (EMAIL-01/02/04, D-08)

**Wave 3** *(blocked on Wave 2 completion)*

- [ ] 01-04-PLAN.md — Wire + serve + verify: `send_notification` registration, `StreamableHTTPHandler` on `/mcp`, handler success/error contract, deliverability + handshake gate (NOTIF-01..05, SEC-02, EMAIL-04)

**Wave 4** *(blocked on Wave 3 completion)*

- [ ] 01-05-PLAN.md — Local dev verify: "Debug mcp-notify" devcontainer breakpoints + local MCP client call to `localhost:8080/mcp` (DEVEX-01)
